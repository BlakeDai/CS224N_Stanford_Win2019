...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device): #  -> torch.Tensor
        ...
        padded_char_indices_sents = pad_sents_char(char_indices_sents, self.char2id['<pad>'])
        
        # print(padded_char_indices_sents)
        sents_var = torch.tensor(padded_char_indices_sents, dtype=torch.long, device=device).permute(1, 0, 2).contiguous()
        return sents_var
...
class Vocab(object):
    ...
    @staticmethod
    def build(src_sents, tgt_sents, vocab_size, freq_cutoff):  # -> torch.Tensor
        """ Build Vocabulary.
