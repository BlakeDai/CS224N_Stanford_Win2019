...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         2. Left Arc
        ###         3. Right Arc
        if len(self.buffer) >= 1 and transition == "S":
            self.stack.append(self.buffer.pop(0))
        elif len(self.stack) >= 2 and transition == "LA":
            dropped = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], dropped))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        next_steps = model.predict(minibatch)
        for i in range(len(minibatch)):  
            minibatch[i].parse_step(next_steps[i])
        j = 0
        for curr_parser in minibatch:
            if not curr_parser.buffer and len(curr_parser.stack) == 1:
